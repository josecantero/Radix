...
// ============================================================================
// CHECKSUM FUNCTIONS (CRC32)
// ============================================================================

// CRC32 calculation for message integrity verification
inline uint32_t calculateChecksum(const std::vector<uint8_t>& payload) {
...
inline bool validateChecksum(const Message& msg) {
    if (msg.payload.empty() && msg.header.checksum == 0) {
        // Empty payload with zero checksum is valid (handshake, etc.)
        return true;
    }
    uint32_t calculated = calculateChecksum(msg.payload);

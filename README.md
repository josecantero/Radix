...
**Soverx** is a fully functional blockchain implementation built from scratch in C++17. It features a custom Proof of Work algorithm using RandomX, a robust P2P networking layer, UTXO-based transaction model inspired by Bitcoin, and an innovative **Peer Witnessing** consensus mechanism to prevent 51% attacks.
...
- ✅ **Base58 Encoding**: Human-readable addresses (e.g., `svx_1A2B3C...`)
...
│   ├── money_util.cpp/h           # Currency formatting (XSV)
...
```bash
git clone https://github.com/yourusername/soverx.git
cd soverx
```
...
This will create the `soverx_node` executable in the `build/` directory.
...
```bash
docker build -t soverx-blockchain .
```
...
```bash
./soverx_node --new-wallet mywallet.wallet
```

**Output:**
```
✅ Nueva wallet creada en: mywallet.wallet
   Direccion: svx_1A2B3C4D5E6F7G8H9I0J
```
...
```bash
./soverx_node --server --port 8080
```

#### Connect to an Existing Peer
```bash
./soverx_node --server --port 8081 --connect 127.0.0.1:8080
```

#### Enable RPC API
```bash
./soverx_node --server --rpc
```
...
```bash
./soverx_node --server --mine --miner-addr svx_1A2B3C4D5E6F7G8H9I0J
```
...
#### Check Balance
```bash
./soverx_node --get-balance svx_1A2B3C4D5E6F7G8H9I0J
```

**Output:**
```
💰 Balance de svx_1A2B3C4D5E6F7G8H9I0J: 150.00000000 XSV
```

#### Send XSV to Another Address
```bash
./soverx_node --send 1000000000 svx_RecipientAddress sender.wallet
```
...
> 💡 **Note**: Amount is in **rads** (1 XSV = 100,000,000 rads)

**Example:**
```bash
# Send 10 XSV
./soverx_node --send 1000000000 svx_9Z8Y7X6W5V4U alice.wallet
```
...
docker run -d \
  -p 8080:8080 \
  -p 8090:8090 \
  -v soverx-data:/soverx/data \
  --name soverx-node \
  soverx-blockchain --server --rpc
```
...
docker run -d \
  -p 8080:8080 \
  -v $(pwd)/my-config.json:/soverx/config.json:ro \
  -v soverx-data:/soverx/data \
  soverx-blockchain --config config.json
```
...
curl -X POST http://localhost:8090/ \
  ...
  -d '{
    "jsonrpc": "2.0",
    "method": "getbalance",
    "params": ["svx_1A2B3C4D5E6F7G8H9I0J"],
    "id": 2
  }'
```
...
Soverx supports JSON configuration files for easier management of complex setups:

```bash
# Run with default config.json
./soverx_node --config config.json

# Run with custom config file
./soverx_node --config my-node.json
```
...
```json
{
  ...
  "mining": {
    "enabled": true,
    "miner_address": "svx_YourAddressHere",
    "threads": 4
  },
  "rpc": {
    "enabled": true,
...
./soverx_node --config config.json --port 9000
```
...
| | `miner_address` | string | "svx_miner_default" | Mining rewards address |
...
> 📝 **Note**: Block halving (3 blocks) and initial reward (50 XSV) are currently hardcoded.
...
Soverx uses **RandomX**, a CPU-optimized PoW algorithm designed to resist ASIC mining:
...
Soverx is a **serious cryptocurrency project** with the ambitious goal of becoming a competitive alternative to Bitcoin. Our vision is to create a decentralized digital currency that serves as both a **medium of exchange for payments** and a **store of value**, with the ultimate direction shaped by community adoption and use cases.
...
While currently in MVP stage, Soverx is built on solid foundations and actively progressing toward production readiness. We welcome contributors who share the vision of creating a truly decentralized, community-owned cryptocurrency.
...
```bash
cd build
./tests/svx_tests --gtest_filter=CryptoTest.*
./tests/svx_tests --gtest_filter=TransactionTest.*
./tests/svx_tests --gtest_filter=BlockTest.*
./tests/svx_tests --gtest_filter=BlockchainTest.*
```
...
```bash
./soverx_node --rpc-genkey "MyAppName" rpc_keys.json
```
...
**List all keys:**
```bash
./soverx_node --rpc-listkeys rpc_keys.json
```

**Revoke a key:**
```bash
./soverx_node --rpc-revokekey <key> rpc_keys.json
```
...
- **GitHub Issues**: [github.com/yourusername/soverx/issues](https://github.com/yourusername/soverx/issues)
